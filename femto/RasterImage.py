...
class RasterImage(RasterImageParameters):
    ...
    def convert_Image_to_path(self,img,display_flag=False):
        ...
        for ii in range(data.shape[0]):
            pixel_line=data[ii,:]
            pixel_line=np.append(pixel_line, False) #appending a final balck pixel, to ensure that the shutter is closed at the end
            ...
            new_GCODE_line=np.array([-1*self.px_to_mm, (ii-1)*self.px_to_mm, 0, self.speed_closed, 0, 0.5, 0, 0]) #first move with closed shutter
            GCODE_array=np.vstack([GCODE_array,new_GCODE_line])
            new_GCODE_line=np.array([-1*self.px_to_mm, ii*self.px_to_mm, 0, self.speed_pos, 0, 0.5, 0, 0]) #first move with closed shutter
            GCODE_array=np.vstack([GCODE_array,new_GCODE_line])

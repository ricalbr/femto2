...
class PGMTrench(PGMCompiler):
    ...
    def write(self, dirname: str = "trench"):
            ...
            with PGMCompiler(col_param) as G:
                ...
                for nbox in range(col.nboxz):
                    for t_index, trench in enumerate(col):
                        ...
                        G.instruction(f'MSGDISPLAY 1, "COL {col_idx + 1:03} TR {t_index + 1:03} LV {nbox + 1:03}, W"\n')
                        G.shutter("OFF")
                        if col.u:
                        ...
                        G.instruction(f'MSGDISPLAY 1, "COL {col_idx + 1:03} TR {t_index + 1:03} LV {nbox + 1:03}, F"\n')
                        if col.u:
        ...
        if self.trench_columns:
            ...
            t_list = [
                    os.path.join(col.base_folder, f"FARCALL{idx + 1:03}.pgm") for idx,
                                                                                  col in enumerate(self.trench_columns)
            ]
...
def _example():
    from src.femto.Cell import Cell
    ...
    for i, wg in enumerate(coup):
        wg.start([-2, -wg.pitch / 2 + i * wg.pitch, 0.035]).linear(increment).sin_mzi(
                (-1) ** i * wg.dy_bend, arm_length=1.0).sin_mzi(
                (-1) ** i * wg.dy_bend, arm_length=1.0).linear(increment).sin_mzi(
                (-1) ** i * wg.dy_bend, arm_length=1.0).linear([wg.x_end, wg.lasty, wg.lastz], mode="ABS")
        wg.end()

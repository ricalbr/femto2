...
class Layer:
    ...
    def __repr__(self) -> str:
        return f'Layer {self.name} @{self.layer}'

    @property
    def objects(self) -> dict[type[femtobj], femtobj]:
        return self._objs
    ...
    def parse_objects(self, unparsed_objects: Any | list[Any]) -> None:
        ...
        If the type of the object matches one of the types of the ``Writer`` registered in the ``Layer`` class,
        the object is added to the ``Writer._obj_list``. If not, a ``TypeError`` is raised.

        Parameters
        ----------
        unparsed_objects : list
            List of object that can be stored in a ``Layer``.
        ...
        unparsed_objects = flatten(listcast(unparsed_objects))
    ...
    def add(self, objs: femtobj | tuple[femtobj] | list[femtobj]) -> None:
        ...
        if all(isinstance(obj, femtobj) for obj in objs):
            self.parse_objects(objs)
        else:
            logger.error(
                'Given objects of the wrong type. Layer objects just accept laserpath- or ' 'trench-derived objects.'
            )
            raise ValueError(
                'Given objects of the wrong type. Layer objects just accept laserpath- or ' 'trench-derived objects.'
            )
...
class Device:
    ...
    def add_to_layer(self, layer_id: int, obj: femtobj | list[femtobj]) -> None:
        ...
        self.layers[layer_id].add(listcast(obj))
    ...
    def export(self, export_dir: str = 'EXPORT', verbose: bool = False, **kwargs) -> None:
        ...
        for layer in self.layers.values():
            ...
            for typ, list_objs in layer.objects.items():
                wr=wrs[typ](self._param, objects=list_objs)
                wr.export(export_dir=export_dir)
...
#
#     def plot2d(
#         self, figure: go.Figure | None = None, show: bool = False, save: bool = False, show_shutter_close: bool = True
#     ) -> go.Figure:
#         """Plot 2D.
#
...
#
#     def plot3d(
#         self, figure: go.Figure | None = None, show: bool = True, save: bool = False, show_shutter_close: bool = True
#     ) -> go.Figure:
#         """Plot 3D.
#
...
def main() -> None:
    ...
    waveguides = Layer(layer=0, name='wgs')
    ...
    # Trench
    # trenches = Layer(layer=1, name='trenches')

...
class Device(PGMCompiler):
    ...
    def plot2d(
        self,
        shutter_close: bool = True,
        wg_style=None,
        sc_style=None,
        mk_style=None,
        tc_style=None,
        pc_style=None,
        gold_layer: bool = False,
        show: bool = True,
        save: bool = False,
    ):
        if wg_style is None:
...
class Cell(Device):
    ...
    def _wg_pgm(self, verbose: bool = True):
        ...
        if nest_level(self.waveguides) > 2:
            raise ValueError(
                f"The waveguide list has too many nested levels ({nest_level(self.waveguides)}. "
    ...
    def _mk_pgm(self, verbose: bool = True):
        ...
        if nest_level(self.markers) > 1:
            raise ValueError(
                f"The markers list has too many nested levels ({nest_level(self.markers)}. " "The maximum value is 1."
            )
        if not self.markers:
            return
...
def _example():
    ...
    for index, wg in enumerate(mzi):
        ...
        wg.start([xi, yi, zi]).linear(increment).sin_mzi((-1) ** index * wg.dy_bend).linear([5, 0, 0]).sin_mzi(
            (-1) ** index * wg.dy_bend
